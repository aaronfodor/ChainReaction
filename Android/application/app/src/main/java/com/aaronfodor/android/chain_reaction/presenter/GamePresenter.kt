 ...
 */
    ...
    fun refreshPlayground(propagation_depth: Int) {
        ...
        if (model.isGameEnded() && propagation_depth == SHOW_CURRENT_PLAYGROUND_STATE) {
            ...
            refreshPlaygroundFields(dimension, stateMatrix)
            view.showCurrentPlayer(Math.abs(model.currentPlayerId!!), model.currentPlayerType == AI)
        }
    ...
    private fun handleGameOver() {
        ...
        view.showCurrentPlayer(Math.abs(model.currentPlayerId!!), isWinnerAI)
        view.showResult(Math.abs(model.currentPlayerId!!), model.playersData!!, model.isAiVsHumanGame(), isWinnerAI)
