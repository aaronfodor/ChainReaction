...
import com.aaronfodor.android.chain_reaction.view.subclass.BaseActivity
...
class SettingsActivity : BaseActivity(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String) {
        ...
        when(sharedPreferences.getBoolean(key, false)){
            true ->{
                AudioService.soundOn()
            }
            false ->{
                AudioService.soundOff()
            }
        }
        ...
        when(key) {
            ...
            KEY_SOUND_ENABLED -> {
                AudioService.refreshAudioFlag(applicationContext)
                changeSettings(sharedPreferences)
                AudioService.refreshAudioFlag(applicationContext)
            }
            KEY_TIME_LIMIT -> {
