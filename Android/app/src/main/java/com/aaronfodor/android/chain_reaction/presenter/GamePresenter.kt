...
import com.aaronfodor.android.chain_reaction.model.task.GameLogicTask
 ...
 */
    ...
    companion object {
         ...
         */
        const val REFRESH_RATE_MILLISECONDS: Int = 250
    }
    ...
    init {
        ...
        view.showStart(Integer.valueOf(players[0][0]!!), model.currentPlayerType == AI)

        gameTask = GameLogicTask(model, this, showPropagation, limitedTimeMode, REFRESH_RATE_MILLISECONDS)
    ...
    fun refreshPlayground(propagation_depth: Int) {
        ...
        if (model.isGameEnded() && propagation_depth == SHOW_CURRENT_PLAYGROUND_STATE) {
            ...
            refreshPlaygroundFields(dimension, stateMatrix)
            view.showCurrentPlayer(abs(model.currentPlayerId!!), model.currentPlayerType == AI, REFRESH_RATE_MILLISECONDS)
        }
    ...
    private fun handleGameOver() {
        ...
        val isWinnerAI = model.currentPlayerType == AI

        val stateMatrix = model.currentPlaygroundInfo()
        view.showCurrentPlayer(abs(model.currentPlayerId!!), isWinnerAI, REFRESH_RATE_MILLISECONDS)
        view.showResult(abs(model.currentPlayerId!!), model.playersData!!, model.isAiVsHumanGame(), isWinnerAI)
        ...
        if(gameType == CUSTOM_GAME){
            ...
            if (model.currentPlayerType == HUMAN) {
    ...
    private fun timeLeftCalculator(propagationDepth: Int) {
        ...
        if (!model.isGameEnded() && limitedTimeMode && model.currentPlayerType == HUMAN) {
