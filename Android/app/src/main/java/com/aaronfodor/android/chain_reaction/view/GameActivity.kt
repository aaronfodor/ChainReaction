...
import com.aaronfodor.android.chain_reaction.presenter.IGameView
...
class GameActivity : AdActivity(), IGameView, View.OnClickListener {
    ...
    private fun createNxMGame(players: ArrayList<String>, showPropagation: Boolean, gifEnabled: Boolean, timeLimit: Boolean,
        ...
        // base dp
        val dpSize = 55
        val dpScaledSize = (dpSize * applicationContext.resources.displayMetrics.density).roundToInt()
    ...
    override fun refreshPlaygroundFieldAt(pos_y: Int, pos_x: Int, color: Int, number: Int, isCloseToExplosion: Boolean, gifEnabled: Boolean): Boolean {
        ...
        if(currentImageId != imageToSet){
            ...
            field.startAnimation(AnimationUtils.loadAnimation(this, android.R.anim.fade_in))
     ...
     * @return      boolean                     True if succeed, false otherwise
     */
    override fun showCurrentPlayer(Id: Int, isAI: Boolean, refreshBackgroundDelayMs: Int): Boolean {
    ...
    private fun playEndGameSound(playerType: Int){
        ...
        when(playerType){
            1 -> {
                AudioService.soundHumanVictory()
                AudioService.soundConfetti()
            }
            else -> {
                AudioService.soundAIVictory()
            }
        }
