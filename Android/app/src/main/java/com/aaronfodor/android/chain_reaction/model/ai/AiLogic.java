...
import com.aaronfodor.android.chain_reaction.TFLiteService;
...
public class AiLogic {
    ...
    private Double[] NeuralNetworkStep(Double[][] input_matrix){
        ...
        float[] actualInput = new float[input_matrix.length * input_matrix[0].length];

        //building-up input
        for(int i = 0; i < input_matrix.length; i++){
            for(int j = 0; j < input_matrix[i].length; j++){
                actualInput[(i*3)+j] = input_matrix[i][j].floatValue();
            }
        }

        //getting specific output
        float[] actualOutput = TFLiteService.INSTANCE.inference(actualInput);
        ...
        for(int i = 0; i < actualOutput.length; i++){
            ...
            if(actualOutput[i] > maximum_prediction && input_matrix[i/3][i%3] >= 0){
                maximum_prediction_index = i;
                maximum_prediction = actualOutput[i];
            }
